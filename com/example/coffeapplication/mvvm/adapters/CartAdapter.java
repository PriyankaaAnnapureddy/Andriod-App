...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.CartViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final CartViewHolder holder, int position) {
        ...
        holder.counter.setText(temp.getCount());
        holder.cost.setText(temp.getCost());
        holder.syrop.setText(temp.getSyrop());

        holder.plus.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                handleItemCountChange(temp, holder, 1);
            }
        });
    ...
    private void handleItemCountChange(Cart cart, CartViewHolder holder, int delta) {
        DatabaseReference rootRef = FirebaseDatabase.getInstance().getReference("Cart");
        rootRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot ds : snapshot.getChildren()) {
                    Cart currentCart = ds.getValue(Cart.class);
                    String id = ds.getKey();

                    if (cart.getName().equals(currentCart.getName())) {
                        int newCount = Integer.parseInt(currentCart.getCount()) + delta;
                        if (newCount > 0) {
                            Map<String, Object> map = new HashMap<>();
                            map.put("count", String.valueOf(newCount));
                            rootRef.child(id).updateChildren(map);
